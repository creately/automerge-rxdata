...
export class Database {
  ...
  public collection<T extends IDocument>(cname: string, opts = {}): Collection<T> {
    const name = `rxdata.${this.name}.${cname}`;
    if (this.collections.has(name)) {
      return this.collections.get(name) as Collection<T>;
    }
    const collection = this.createCollection<T>(name, opts);
  ...
  public async drop(): Promise<void> {
    this.collections.forEach( async c => await c.remove({}));
    this.collections = new Map();
  ...
  protected createCollection<T extends IDocument>(name: string, opts = {}): Collection<T> {
    this.registerCollection(name);
    return new Collection<T>(name, opts);
  }
}
